...
<div class="registeer">
  <div class="container"> 
    <div class="row pt-5 ">
    <form class="form col-12 col-md-6 offset-md-3 p-5" method="POST" action="{{route('auth.handle-register')}}">
        @csrf
        <div class="mb-3">
          <label for="name" class="form-label">{{__('site.name')}}</label>
          <input type="text" name="name" placeholder="{{__('site.Enter Your Name')}}" class="form-control" id="name" value="{{old('name')}}" >
          @error('name')
        ...
        <button type="submit" class="btn main-btn d-block w-100 ">{{__('site.register')}}</button>
      </form>
  </div>
  <div class="row">
